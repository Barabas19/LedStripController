...
#define PRECISE_ALGORITHM	// тик 16 мкс, медленнее, но работает даже с убитым энкодером (по мотивам https://github.com/mathertel/RotaryMCP_Encoder)
...
// MCP_Encoder enc;									// не привязан к пину
// MCP_Encoder enc(пин CLK, пин DT);				// энкодер без кнопки (ускоренный опрос)
// MCP_Encoder enc(пин CLK, пин DT, пин SW);		// энкодер с кнопкой
// MCP_Encoder enc(пин CLK, пин DT, пин SW, тип);	// энкодер с кнопкой и указанием типа
// MCP_Encoder enc(пин CLK, пин DT, ENC_NO_BUTTON, тип);	// энкодер без кнопкой и с указанием типа

class MCP_Encoder {
public:
	MCP_Encoder();								// для непривязанного к пинам энкодера
	MCP_Encoder(Adafruit_MCP23017 mcp, uint8_t clk, uint8_t dt, int8_t sw = -1, bool type = false); // CLK, DT, SW, тип (TYPE1 / TYPE2) TYPE1 одношаговый, TYPE2 двухшаговый. Если ваш энкодер работает странно, смените тип
